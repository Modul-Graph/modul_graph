...
def get_start_competences_plus_sems_and_obl_mods_plus_sems() -> tuple[dict[str, int], dict[str, list[int]]]:
    ...
    for module in obligatory_modules:
        ...
        for comp in comps:
            ...
            if comp not in competences_plus_semester.keys() or competences_plus_semester[
                comp] > semester_of_comp_provision:
...
def does_feasible_subgraph_exist(standard_curriculum: str) -> bool:
    instantiate_std_curr_obj(standard_curriculum)
    start_comps_plus_sem_and_obl_mods_plus_sems: tuple[
        dict[str, int], dict[str, list[int]]] = get_start_competences_plus_sems_and_obl_mods_plus_sems()
...
def __get_subgraph_for_feasibility_analysis(wanted_comp: str, start_comps_plus_sem_and_obl_mods_plus_sems: tuple[
    dict[str, int], dict[str, list[int]]]) -> list[tuple[str, str, list[int], list[str]]]:
    ...
    for curr_sem in range(lowest_sem_of_free_slot, highest_sem_of_free_slot + 1):
        ...
        limited_comps: list[str] = [competence_pool[i] for i in range(len(competence_pool)) if
                                    provided_in_sem[i] <= curr_sem]
        ...
        possible_modules = [item for item in possible_modules if
                            set(da_get_needed_comps_for_module(item)) <= set(limited_comps)]
        ...
        found_modules_sems: list[list[int]] = []

        # try 10 times to find fitting subgraph
        for i in range(0, 10):
            found_modules, found_modules_slot_types, found_modules_sems, types_of_free_slots, winter_of_free_slots, sems_of_free_slots = __fit_possible_modules_to_free_slots(
                wanted_comp, possible_modules, list(types_of_free_slots), list(winter_of_free_slots),
                list(sems_of_free_slots), curr_sem)
        ...
        existing_mods: list[str] = [x for i, x in enumerate(subgraph[0]) if
                                    min(subgraph[2][i]) < curr_sem and min(subgraph[2][i]) != -1]

        subgraph[3].extend(__compute_links(found_modules, existing_mods))
        ...
        comps_of_found_modules, sems_of_comp_provision = da_get_provided_comps_for_module_list_plus_sem_of_provision_without_duplicates(
            found_modules)
        ...
        indices_containing_duplicates: list[int] = [i for key in
                                                    (key for key, count in Counter(competence_pool).items() if
                                                     count > 1) for i, x in enumerate(competence_pool) if x == key]
        first_index_per_duplicate: list[int] = [competence_pool.index(key) for key, count in
                                                Counter(competence_pool).items()]
...
def __get_free_slots_by_type_plus_semester_plus_season() -> tuple[list[list[str]], list[int], list[bool]]:
    ...
    if len(free_slots_ids) != 0:
        _s1: list[tuple[str, list[str], int, bool]] = list(zip(free_slots_ids, free_slots_types, semesters, winter))
        _s2 = sorted(_s1, key=lambda x: x[2])
        _s3 = list(zip(*_s2))
        free_slots_ids, free_slots_types, semesters, winter = list(_s3[0]), list(_s3[1]), list(_s3[2]), list(_s3[3])

        # sorted_by_sem: tuple[list[str], list[list[str]], list[int], list[bool]] = zip(
        #     *sorted(zip(free_slots_ids, free_slots_types, semesters, winter), key=lambda x: x[2]))
...
def __get_next_level_modules_plus_areas(comps: list[str], existing_mods: list[str]) -> tuple[
...
def __fit_possible_modules_to_free_slots(wanted_comp: str, possible_modules: list[str],
                                         types_of_free_slots: list[list[str]], winter_of_free_slots: list[bool],
                                         sems_of_free_slots: list[int], curr_sem) -> tuple[
    list[str], list[str], list[list[int]], list[list[str]], list[bool], list[int]]:
    ...
    for i, slot in enumerate(types_of_free_slots.copy()):
        ...
        for j, single_slot_type in enumerate(slot):
            ...
            if found_module != '':
                ...
                break
        if found_module == '':
            remaining_free_slots, and_their_winter, and_their_sem = __remove_indices(types_of_free_slots,
                                                                                     winter_of_free_slots,
                                                                                     sems_of_free_slots,
                                                                                     indices_to_remove_from_free_slot_lists)
            return found_modules, found_modules_slot_types, found_modules_sem, remaining_free_slots, and_their_winter, and_their_sem

    remaining_free_slots, and_their_winter, and_their_sem = __remove_indices(types_of_free_slots, winter_of_free_slots,
                                                                             sems_of_free_slots,
                                                                             indices_to_remove_from_free_slot_lists)
    return found_modules, found_modules_slot_types, found_modules_sem, remaining_free_slots, and_their_winter, and_their_sem


def __remove_indices(list1: list[list[str]], list2: list[bool], list3: list[int], indices: list[int]) -> tuple[
    list[list[str]], list[bool], list[int]]:
