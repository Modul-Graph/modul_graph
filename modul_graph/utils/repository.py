...
def get_provided_comps_for_module_list(modules: list[str]) -> list[str]:
...
def get_possible_modules_plus_provided_comps_via_existing_comps(comps: list[str]) -> list[str]:
...
def get_possible_modules_via_existing_comps(comps: list[str]) -> list[str]:
    comps = prepare_list_as_cypher_var(comps)
    return db.cypher_query('MATCH (c:Competence)<-[:NEEDS]-(m:Module) WHERE c.name in [' + ', '.join(comps) + '] RETURN m.name')


def prepare_list_as_cypher_var(input_list: list[str]) -> list[str]:
...
def get_module_areas_for_optional_modules(obl_module_areas: list[str]) -> list[str]:
...
def get_module_cells_connected_to_module_area(module_area: list[str]) -> list[str]:
